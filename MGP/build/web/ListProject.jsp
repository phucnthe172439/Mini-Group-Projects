                ...
                <div class="col-xs-12">
                    <div class="box">
                        <div class="box-header" style="margin-bottom: 10px;margin-top:20px;">
                            <h3 class="box-title" style="font-size: 35px; font-weight: bold;"><span>Project List</span></h3>
                        </div>
                            ...
                            <form action="projectList">
                                <div class="row">
                                    ...
                                    <div class="input-group col-4">
                                        <select class="form-control" name="groupID" style="height: 32px">
                                            <option value="0">All</option>
                                            <c:forEach var="item" items="${requestScope.groups}">
                                                <option value="${item.id}" ${requestScope.groupID == item.id? 'selected' : ''}>${item.name}</option>
                                            </c:forEach>
                                        </select>
                                    </div>
                                    <div class="col-auto">
                        ...
                        <div class="box-body table-responsive no-padding">
                            <table class="table table-hover" style="margin: 0 auto; width: 90%;">
                                <tr>
                                    ...
                                    <th style="width: 20%;text-align: center">English Name</th>
                                    <th style="width: 10%;text-align: center">Vietnamese Name</th>
                                    <th style="width: 10%;text-align: center">Status</th>
                                    <th style="width: 10%;text-align: center">Group</th>
                                    <th style="width: 25%;text-align: center">Actions</th>
                                </tr>
                                <c:forEach items="${requestScope.projects}" var="u">
                                    <tr>
                                        <td style="width: 5%;text-align: center">${u.id}</td>
                                        <td style="width: 20%;text-align: center">${u.code}</td>
                                        <td style="width: 20%;text-align: center">${u.engname}</td>
                                        <td style="width: 10%;text-align: center">${u.viname}</td>
                                        <td style="width: 10%;text-align: center">
                                            ${u.status eq true ? '<span style="color: green;">Active</span>' : '<span style="color: red;">Deactive</span>'}
                                        </td>
                                        <td style="width: 10%;text-align: center">${u.group.name}</td>
                                        ...
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    Do you want to delete project #${u.code}
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                                    <button type="button" onclick="window.window.location.href = '${pageContext.request.contextPath}/deleteProject?id=${u.id}'" class="btn btn-primary">Save changes</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="modal fade" id="exampleModal${u.id}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form action="${pageContext.request.contextPath}/projectList" method="post">
                                                        <input type="hidden" name="id" value="${u.id}">
                                                        <input type="hidden" name="action" value="update">
                                                        <div class="form-group">
                                                            <label for="recipient-code" class="col-form-label">Code</label>
                                                            <input type="text" class="form-control" id="recipient-code" name="code" value="${u.code}" readonly="" required="" >
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="recipient-name" class="col-form-label">English Name</label>
                                                            <input type="text" class="form-control" id="recipient-name" name="englishname" value="${u.engname}" required="">
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="recipient-name" class="col-form-label">Vietnamese Name</label>
                                                            <input type="text" class="form-control" id="recipient-name" name="viname" value="${u.viname}" required="">
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="subject" class="col-form-label">Group</label>
                                                            <select class="form-control" name="groupID" style="height: 32px">
                                                                <c:forEach var="item" items="${requestScope.groups}">
                                                                    <option value="${item.id}" ${u.group.id == item.id? 'selected' : ''}>${item.name}</option>
                                                                </c:forEach>
                                                            </select>
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="subject" class="col-form-label">Description</label>
                        ...
                        <div class="box-footer clearfix" style="margin-left: 44%">
                            <ul class="pagination pagination-sm no-margin pull-left text-center">
                                <c:if test="${requestScope.pagenum > 1}">
                                    <li class="page-item"><a style="font-size: larger;" class="page-link" href="${pageContext.request.contextPath}/projectList?pagenum=${requestScope.pagenum - 1}">Previous</a></li>
                                    </c:if>
                                    ...
                                    <li class="page-item"><a style="font-size: larger;" class="page-link" href="${pageContext.request.contextPath}/projectList?pagenum=${i}">${i}</a></li>
                                    </c:forEach>
                                    <c:if test="${requestScope.pagenum < requestScope.totalPage}">
                                    <li class="page-item"><a style="font-size: larger;" class="page-link" href="${pageContext.request.contextPath}/projectList?pagenum=${requestScope.pagenum + 1}">Next</a></li>
                                    </c:if>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form action="${pageContext.request.contextPath}/projectList" method="post">
                                        ...
                                        <div class="form-group">
                                            <label for="recipient-name" class="col-form-label">English Name</label>
                                            <input type="text" class="form-control" id="recipient-name" name="englishname" required="">
                                        </div>
                                        <div class="form-group">
                                            <label for="recipient-name" class="col-form-label">Vietnamese Name</label>
                                            <input type="text" class="form-control" id="recipient-name" name="viname" required="">
                                        </div>
                                        <div class="form-group">
                                            <label for="subject" class="col-form-label">Group</label>
                                            <select class="form-control" name="groupID" style="height: 32px">
